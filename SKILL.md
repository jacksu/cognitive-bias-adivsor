...
- **决策应用**："决策""决定""选择""我要不要…""帮我决策""帮我判断""决策质量""判断偏差"
...
1. **识别领域**：根据用户关键词匹配领域（见下方路由表）
2. **加载知识库**：用 `read_file` 加载对应 `references/domain-{领域}.md`
3. **输出结果**：先复述底线 → 输出该领域的核心偏差表 + 应对策略 + 话术 → 末尾附反向自检

### 输出模板

加载 `domain-{领域}.md` 后，按以下 Markdown 模板输出，不得省略任何章节：
...
**约束**：
- 话术必须用 `>` 引用块，`[xxx]` 为需替换变量
- 策略数量与 domain 文件一致，不增不减
- 领域名从路由表取，偏差表/策略/话术/反向自检全部取自 domain 文件

### 领域路由与知识库文件

| # | 领域 | 触发关键词 | 参考文件 |
|---|------|-----------|---------|
| 1 | **决策** | 决策/决定/选择/我要不要/帮我决策 | `references/domain-decision.md` |
| 2 | **学习** | 学习/怎么学/记不住/学不会/效率低/进步 | `references/domain-learning.md` |
| 3 | **行动** | 拖延/不想动/执行/行动力/开始做/坚持 | `references/domain-action.md` |
| 4 | **沟通** | 说服/谈判/激励/沟通技巧/怎么推动 | `references/domain-communication.md` |
| 5 | **影响** | 影响别人/推动改变/塑造/引导/助推 | `references/domain-influence.md` |
| 6 | **管理** | 管理团队/领导/带人/招聘/绩效/考核 | `references/domain-management.md` |

> 若触发决策领域关键词，优先走 [Phase 1](#phase-1-收集决策问题) → Phase 2 十问框架，而非直接输出快速参考。
